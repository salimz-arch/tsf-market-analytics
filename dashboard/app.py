...
def load_raw():
    ...
    for f in sorted(RAW_DIR.glob("*.csv")):
        try:
            ...
            continue
        if not {"series_id", "value", "timestamp"}.issubset(df.columns):
            continue
        df["timestamp"] = pd.to_datetime(df["timestamp"], errors="coerce")
        df = df.dropna(subset=["series_id", "value", "timestamp"])
        df["series_id"] = df["series_id"].astype(str).str.strip()
        df = df[~df["series_id"].str.match(r"^\d+(\.\d+)?$")]
        df = df[df["series_id"] != ""]
        if "domain" in df.columns:
            df["domain"] = df["domain"].astype(str).fillna("unknown")
        else:
            df["domain"] = "unknown"
        frames.append(df)
...
def latest_fc(fc):
    if fc.empty or "created_at" not in fc.columns or "series_id" not in fc.columns:
        return fc
    return fc.sort_values("created_at").groupby("series_id").tail(1)
...
def filter_tf(df, tf):
    ...
    days = {"7D": 7, "30D": 30, "90D": 90}.get(tf, 90)
    max_ts = df["timestamp"].max()
    if pd.isna(max_ts):
        return df
    return df[df["timestamp"] >= max_ts - pd.Timedelta(days=days)]
...
n_anom = int(is_anom(lat["warning"]).sum()) if not lat.empty and "warning" in lat.columns else 0
...
if page == "🏠 Overview":
    ...
    k = st.columns(4)
    mape_avg = float(mt["mape_pct"].mean()) if not mt.empty and "mape_pct" in mt.columns else np.nan
    cov = ev["within_80"].dropna().mean() * 100 if not ev.empty and "within_80" in ev.columns else np.nan
    n_series = len(raw["series_id"].unique()) if not raw.empty else 0
    n_domain = raw["domain"].nunique() if not raw.empty else 0
    k[0].metric("Series Dipantau", n_series, f"{n_domain} domain")
    k[1].metric("Anomali Aktif", n_anom, "normal" if n_anom == 0 else "perlu perhatian", delta_color="inverse")
    k[2].metric("MAPE Rata-rata", f"{mape_avg:.2f}%" if pd.notna(mape_avg) else "-", "validasi")
    k[3].metric("Coverage Live", f"{cov:.0f}%" if pd.notna(cov) else "-", "target 80%")

    # ---- BANNER PERINGATAN DINI ----
    act = lat[is_anom(lat["warning"])] if not lat.empty and "warning" in lat.columns else lat.head(0)
    ...
    with g[0]:
        ...
        series_list = sorted(raw["series_id"].unique().tolist()) if not raw.empty else []
        sel = cc1.selectbox("Series", series_list, label_visibility="collapsed") if series_list else None
    ...
    with t[1]:
        ...
        if not lat.empty and "change_pct" in lat.columns:
            lat_clean = lat.copy()
            lat_clean["change_pct"] = pd.to_numeric(lat_clean["change_pct"], errors="coerce")
            lat_clean = lat_clean.dropna(subset=["change_pct"])
            if not lat_clean.empty:
                top = lat_clean.reindex(lat_clean["change_pct"].abs().sort_values(ascending=False).index).head(5)
                rows = ""
                ...
                st.markdown("<div class='feed-row muted'>Belum ada data perubahan.</div>", unsafe_allow_html=True)
...
# ================= FORECAST & INTERVAL =================
    ...
    cols = [c for c in ["series_id", "domain", "last_value", "predicted_next_value", "lower_bound", "upper_bound", "change_pct", "signal", "warning"] if c in sub.columns]
    st.dataframe(sub[cols].sort_values("series_id"), use_container_width=True, hide_index=True)
...
# ================= LAPORAN AI =================
    ...
    if not cc.empty:
        last = cc.iloc[-1]
        engine = last.get("engine", "unknown")
        ...
        with st.expander("Riwayat laporan"):
            for _, r in cc.sort_values("created_at", ascending=False).head(10).iterrows():
                ...
                st.markdown(f"**{ts}** • {r.get('engine', 'unknown')}")
